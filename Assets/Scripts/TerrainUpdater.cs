...
public class TerrainUpdater : MonoBehaviour
{
    ...
    {
        toRender.Add(GenerateTerrainMesh(x, z));
    }

    ChunkInstance GenerateTerrainMesh(int chunkx, int chunkz)
    {
        ...
        return new ChunkInstance { mesh = m.ToMesh(), transform = Matrix4x4.Translate(new Vector3(chunkx << Chunk.CHUNK_X_SHIFT, 0, chunkz << Chunk.CHUNK_Z_SHIFT)) };
    }
}
